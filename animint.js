...
window.__animint_testing = window.location.href.includes("testthat") || 
                         /chromote/i.test(navigator.userAgent);
  ...
  */
  ...
  var draw_geom = function(g_info, chunk, selector_name, PANEL){
	...
	text_fun = function(d){
      ...
      //   hideOnClick: true,
      //   onShow(instance) {
      //     // Prevent tooltips during Chromote operations
      //     if (window.__chromote_active) {
      //       return false;
      //     }
      //     return true;
      //   }
      ...
      if (window.__animint_testing) {
        elements.attr("title", text_fun);
      } 
      // For normal usage, use Tippy.js
      else {
        elements.attr("data-tippy-content", text_fun);
        tippy("[data-tippy-content]", {
          allowHTML: true,
